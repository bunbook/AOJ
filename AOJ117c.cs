...
namespace AOJ
{
    class AOJ117c
    {
        ...
        static void Main(string[] args)
        {
            ...
            bool[] passed = new bool[tawnNum];

            int trafficCost = 0;
            trafficCost += CulcCost(startTawnId, goalTawnId, 0, passed);
            trafficCost += CulcCost(goalTawnId, startTawnId, 0, passed);
        ...
        static int CulcCost(int current, int goal, int currentTotalCost, bool[] passed)
        {
            if (current == goal)
            {
                /*
            ...
            bool[] newPassed = (bool[])passed.Clone();
            newPassed[current] = true;

            int cheapestCost = INF;
            for (int i = 0; i < newPassed.Length; i++)
            {
                if (pathCosts[current, i] == NAN || newPassed[i])
                    continue;
                int totalCost = currentTotalCost + pathCosts[current, i];
                totalCost = CulcCost(i, goal, totalCost, newPassed);
