...
namespace AOJ
{
    ...
    /// <summary>
    /// コメントや冗長なコード有り 00:02s（コメ等無しだと 00:01s
    /// </summary>
    class AOJ156b
    {
    ...
    class Ninja
    {
        ...
        private bool DFS(int[] posYX, char targetStatus)
        {
            ...
            // if (areaMemo[posYX[0], posYX[1]] > NEXT)
                return false;
        ...
        /// ちゃんとStack用いる深さ優先
        /// </summary>
        private bool DFS(int[] posYX, Stack<int[]> openNodes, char targetStatus)
        {
            // if (areaMemo[posYX[0], posYX[1]] != NONE && areaMemo[posYX[0], posYX[1]] != NEXT)
            if (areaMemo[posYX[0], posYX[1]] > NEXT)
                return false;
            ...
            for (int i = 0; i < 4; i++)
            {
                ...
                if (map.status[posY, posX] == targetStatus)
                {
                    openNodes.Push(new int[2] { posY, posX });
                }
                else if (map.status[posY, posX] == Map.GOAL)
                {
                    return true;
            ...
            {
                if (DFS(openNodes.Pop(), openNodes, targetStatus))
                    return true;
            }
            return false;
        }
